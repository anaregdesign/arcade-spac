...
import { getIrPrimerUiCopy } from "./copy";
...
export function useIrPrimerWorkspace(workspace: GameWorkspaceController, locale: SupportedArcadeLocale) {
  ...
  const content = useMemo(() => getIrPrimerContent(locale), [locale]);
  const sections = useMemo(() => getIrPrimerSections(content), [content]);
  const copy = useMemo(() => getIrPrimerUiCopy(locale), [locale]);
  const timeLimitSeconds = irPrimerTimeLimitByDifficulty[workspace.difficulty];
