...
## Component CSS Module File Naming

### Summary

component 専用の CSS Module file は、対応する component `.tsx` file と同じ `UpperCamel` basename に揃える。

### User Problem

- component file だけ `UpperCamel` でも CSS Module が `kebab-case` のままだと、対応関係が file 名から一目で分からない
- component rename のたびに CSS Module だけ naming pattern がずれると、search と rename がやりづらい
- shared style asset と component-owned style asset の naming rule が混ざると ownership が曖昧になる

### Users and Scenarios

- 開発者は `ComponentName.tsx` を開いたとき、隣の `ComponentName.module.css` をすぐ特定したい
- 開発者は component rename と CSS Module rename を同じ change set で扱いたい
- レビュアーは file 名だけで component-owned style か shared style asset かを判別したい

### Scope

- `app/components/` 配下で 1 component に紐づく `.module.css` file を、対応する `.tsx` file と同じ basename に統一する
- rename 後の CSS Module import path を repository 全体で整合させる
- shared CSS Module で component 1:1 対応ではないものは既存名を維持できるようにする

### Non-Goals

- global stylesheet や route-owned stylesheet の naming 変更
- CSS selector 名や style 内容の変更
- component をまたぐ shared CSS Module の basename を無理に component 名へ寄せること

### User-Visible Behavior

- 画面表示や style の適用結果は rename 前後で変わらない
- component file と component-owned CSS Module file は basename が一致する
- `GameWorkspaceShared.module.css` のような shared CSS Module は shared asset として既存 naming を維持できる

### Acceptance Criteria

- component 1:1 対応の `.module.css` file は対応する `.tsx` file と同じ basename になる
- rename 対象 CSS Module の import path は全て更新される
- shared CSS Module と global stylesheet は existing role-based naming を維持できる
- build と typecheck が通る

### Edge Cases

- `ResultScreen.tsx` のように component が subdirectory にあっても `ResultScreen.module.css` を使う
- game-specific workspace component でも CSS Module は `ColorSweepGameWorkspace.module.css` のように component 名に揃える
- shared CSS Module が複数 component をまたぐ場合は、component 名へ rename せず shared asset 名を維持してよい

### Constraints and Dependencies

- component-owned CSS Module rename は対応する component import update と同じ change set で完結させる
- route module naming と global stylesheet naming はこの rule の対象外とする

### Links

- Related: [#component-tsx-file-naming](#component-tsx-file-naming)
