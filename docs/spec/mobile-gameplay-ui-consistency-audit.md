...
見た目だけでなく 50 game を actual play し、各 game ごとの operability、UI 違和感、rule cue の過不足、過度な空白、overflow を 1 件ずつ洗い出したうえで、shared layout archetype と shared sizing strategy へまとめて修正する。
...
- 開発者は 50 game を actual play しながら、`操作はできるが気持ち悪い`, `rule cue が伝わりづらい`, `余白で board が小さい`, `要素が大きすぎて viewport を圧迫する` といった半故障状態も拾いたい
- 単純な screen width 比例だけで size を決めると、touch target や desktop balance が壊れやすく、game ごとの差も吸収しづらい
...
- 各 game は idle state だけでなく live state でも actual play し、`start`, `主要 input`, `1 手以上の反応`, `終了または failure state 到達` を確認する
- audit 中に見つかった per-game findings は execution artifact に 1 件ずつ残し、shared へ寄せられる問題か game-specific かを分類する
- size tuning は raw `vw` や screen width 比例 1 本ではなく、`clamp()`, CSS custom properties, min/max touch-safe constraints, layout archetype ごとの container rule を組み合わせる
...
- 50 game すべてで `actual play` の所見が残り、`操作不能`, `分かりにくい`, `余白過多`, `過大要素`, `overflow`, `rule cue mismatch` などの findings が未整理のまま残らない
- shared layout archetype ごとに size と spacing の scale token が揃い、board, dock, sidecar, choice grid の寸法は viewport と container に対して滑らかに追従する
...
- 50 game すべてで actual play により `start`, `主要 input`, `1 手以上の live reaction`, `終了または正常 failure` が確認され、結果は execution artifact に記録されている
- audit 中に見つかった findings は game ごとに列挙され、shared cluster と game-specific cluster に分類されている
- size system は raw `vw` 依存ではなく `clamp()` + custom property + touch-safe min/max rule で共有され、少なくとも layout variant で共通に使われている
...
- screen width 比例だけに寄せると tablet や desktop で巨大化しやすく、narrow mobile では touch-safe minimum を下回りやすいので、min/max clamp を前提にする
...
- size strategy の共有化で新しい external dependency が必要かは audit 後に判断し、`clamp()` と existing CSS/custom property で十分なら dependency は増やさない
