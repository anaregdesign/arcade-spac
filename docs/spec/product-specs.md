...
## Multilingual Game Guidance

### Summary

`Arcade` はゲーム名の表記は維持しつつ、ゲーム説明と `How to play` を利用者が選んだ言語、または browser 言語設定に合わせて表示する。

### User Problem

- 現状のゲーム説明と `How to play` は英語固定で、非英語話者がゲーム内容を素早く把握しにくい
- 一覧画面、ゲーム画面、結果画面で同じゲーム説明が出てきても、言語が統一されないと理解コストが高い
- 言語切替が画面ごとに分かれていると、毎回設定し直す必要があり体験が不安定になる

### Users and Scenarios

- 日本語利用者が Home のゲーム説明を日本語で読み、そのまま `How to play` も日本語で確認してプレイを始める
- 中国語利用者が browser 言語を中国語に設定した状態で初回訪問し、追加設定なしで説明文を読める
- 英語またはフランス語利用者がトップのメニューバーから明示的に表示言語を切り替える

### Scope

- 対応言語は `browser default`、日本語、中国語、英語、フランス語とする
- Home、Game、Result のゲーム説明を選択言語で表示する
- 各ゲームの `How to play` ダイアログを選択言語で表示する
- トップのメニューバーから表示言語を切り替えられるようにする
- 初回表示や `browser default` 選択時は browser 言語設定を参照して最適な対応言語へ解決する

### Non-Goals

- ゲーム名そのものを翻訳またはローカライズすること
- 全アプリ UI 文言を今回の変更で完全多言語化すること
- 非対応言語をその場で機械翻訳して表示すること

### User-Visible Behavior

- トップのメニューバーに表示言語 selector があり、利用者は `browser default`、日本語、中国語、英語、フランス語を選べる
- `browser default` では browser の言語設定から `ja`、`zh`、`en`、`fr` のいずれかへ解決し、未対応言語は英語へフォールバックする
- Home のゲームカード説明、Game 画面の説明、Result 画面の推薦説明は選択言語で揃う
- `How to play` ダイアログは選択言語で開き、同じゲームでも切替後は新しい言語で表示される
- 言語選択はページ遷移後も維持される

### Acceptance Criteria

- メニューバーから言語を切り替えると、次の描画からゲーム説明と `How to play` が選択言語で表示される
- `browser default` を選んだ状態で browser 言語が `ja`、`zh`、`en`、`fr` のいずれかなら、その言語が自動選択される
- browser 言語が未対応の場合は英語表示になる
- ゲーム名はどの言語でも既存表記を維持する
- 言語選択は cookie などの継続設定で保持され、別画面へ遷移しても失われない

### Edge Cases

- `zh-CN` や `zh-TW` のような地域付き言語タグでも中国語として扱う
- `en-US`、`fr-CA`、`ja-JP` のような地域付き言語タグでも対応基底言語へ解決する
- 翻訳データが欠けている項目は英語へフォールバックし、画面を壊さない
