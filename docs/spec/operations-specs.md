...
`Arcade` の Azure hosting を、Azure SQL public endpoint 依存から `Private Endpoint` 前提の構成へ切り替えつつ、runtime config sync は GitHub Actions OIDC 中心で回る運用手順へ揃える。
...
- App Configuration と Key Vault の update path は workflow 側へ寄せられたが、resource group 作成と initial Azure SQL principal bootstrap はまだ dedicated bootstrap workflow で閉じていなかった
...
- App Configuration と Key Vault の hosted access を GitHub-hosted workflow から OIDC sync できる contract へ更新する
- app runtime の config bootstrap を Azure App Configuration と Key Vault を source of truth とする形へ寄せる
- Container App の health probe と production verification を current connectivity contract へ更新する
...
- routine redeploy では GitHub Release workflow が runtime config を App Configuration / Key Vault へ同期する
- README と operations docs を読むと、Azure SQL public access を戻すのではなく private path を維持・検証する運用手順が示される
- production verification は Azure SQL private connectivity と config store wiring contract に沿って pass/fail する
...
- hosted App Configuration と Key Vault が GitHub Actions OIDC sync と runtime RBAC access を前提にした contract へ更新されている
- server-side runtime config bootstrap が Azure App Configuration と Key Vault を使って config を解決できる
- runtime config を stores へ同期する workflow path と workflow-owned bootstrap path が用意されている
- Container App の health probe が `/health` に向く explicit HTTP probe として定義されている
- production verification script が public firewall contract ではなく hosted Azure contract を検証する
...
- SQL admin login/password は bootstrap-only recovery secret とし、runtime auth に流用しない
- runtime identity と migration identity の分離は維持し、runtime identity に DDL 系 privilege を与えない
- config store sync に必要な data-plane write 権限は GitHub OIDC identity へ付与するが、routine redeploy に local private network host を要求しない
...
- live Azure resource への config 同期と bootstrap は GitHub Actions OIDC workflow path を primary にする
...
- production release 前に workflow-driven validation を通し、その結果を踏まえて GitHub release workflow 経由で deploy する
...
- release 対象 commit に対して workflow-driven validation の成功が確認されている
