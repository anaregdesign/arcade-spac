...
- [x] Maintain this execution plan during the audit by appending newly discovered tasks and retaining completed checkboxes until archive.
...
- [x] Add a shared compact pattern for summary-heavy and action-heavy board cards that overrun the mobile viewport.
- [x] Reorder or compress tray-first and side-panel-first layouts that push the primary board below the fold on mobile.
- [x] Normalize tap-answer and tap-lane button placement patterns across games that rely on repeated primary button taps.
- [x] Bias mobile primary button placement toward the lower part of the viewport for games that use separate tap buttons outside the board itself.
- [x] Keep twin-board and twin-panel comparison games compact enough to stay horizontally paired on mobile when a stacked layout would push gameplay below the fold.
- [x] Prevent answer-choice and clue-card layouts from falling back to single-column mobile stacks when two compact columns keep the game playable without shrinking touch targets too far.
- [x] Compress side insight panels on graph, timing, and zone-planning games so auxiliary copy does not outrun the playable board on either mobile or desktop.
- [x] Normalize game title / description blocks into one compact shared gameplay heading and drop redundant explanatory copy on mobile where the help affordance already covers it.
- [x] Move redundant board-level rule copy into HowToPlay and keep only live gameplay cue text in the workspace itself.
- [x] Remove duplicate in-board summary strips from summary-heavy workspaces when the controls card already exposes the same run status and progress.
- [x] Collapse auxiliary legends, meters, and side panels when they outrun the primary playable board on mobile.
...
- [x] Evaluate any remaining instruction-heavy outliers after copy reduction and record whether rule simplification is actually necessary.
- [x] Classify the 50 game workspaces into shared gameplay layout archetypes and record which games map to each archetype.
...
- [x] Migrate touched game workspaces away from one-off structural CSS and onto the new shared layout variants where the archetype matches.
...
- [x] Audit and repair `Path Recall` on mobile and desktop
- [x] Audit and repair `Position Lock` on mobile and desktop
- [x] Audit and repair `Pulse Count` on mobile and desktop
- [x] Audit and repair `Quick Sum` on mobile and desktop
- [x] Audit and repair `Sum Grid` on mobile and desktop
- [x] Audit and repair `Symbol Hunt` on mobile and desktop
- [x] Audit and repair `Hidden Find` on mobile and desktop
- [x] Audit and repair `Hue Drift` on mobile and desktop
- [x] Audit and repair `Spot Change` on mobile and desktop
- [x] Audit and repair `Tap Safe` on mobile and desktop
- [x] Create a reviewable commit for shared or game-specific fixes discovered in games 31-40.

## Section 6 - Batch 5 mobile audit and repair
### Subsection 6.1 - Games 41-50
- [x] Audit and repair `Light Grid` on mobile and desktop
- [x] Audit and repair `Flip Match` on mobile and desktop
- [x] Audit and repair `Rotate Align` on mobile and desktop
- [x] Audit and repair `Tile Shift` on mobile and desktop
- [x] Audit and repair `Swap Solve` on mobile and desktop
- [x] Audit and repair `Tile Instant` on mobile and desktop
- [x] Audit and repair `Zone Lock` on mobile and desktop
- [x] Audit and repair `Stack Sort` on mobile and desktop
- [x] Audit and repair `Mirror Match` on mobile and desktop
- [x] Audit and repair `Shape Morph` on mobile and desktop
- [x] Create a reviewable commit for shared or game-specific fixes discovered in games 41-50.

## Section 7 - Cross-game verification and closeout
- [x] Re-run the full 50-game mobile and desktop sweep after shared cue normalization to identify the new outlier set under the reduced-copy baseline.
- [x] Re-audit batches 1-3 under the reduced-copy and shared-cue baseline so previously completed games are re-validated against the new standard.
- [x] Re-check representative desktop and mobile routes after all repairs.
- [x] Run `npm run typecheck` and any targeted verification needed for touched game workspaces.
