...
- [x] Confirm the current app implementation exposes 49 playable games in `supportedGames` and `game-workspace-registry`.
...
- [x] Treat the remaining implementation scope as 1 game: `Gap Rush`.
...
- [ ] Reconfirm `/docs/spec/games/gap-rush.md` so the continuous-feel lane drift, wall timing, and result metrics are explicit.
...
- [ ] Add `Gap Rush` to the game catalog metadata and Home preview mapping.
- [ ] Add the game registry entry, help content, and preview asset.
- [ ] Implement the deterministic corridor runner and `Gap Rush` session/workspace Hook under `app/lib/client/usecase/game-workspace/`.
- [ ] Implement the `Gap Rush` workspace Component and CSS under `app/components/games/gap-rush/`.
- [ ] Add deterministic Playwright hooks for avatar lane, incoming gap, perfect-pass state, and run completion without changing the visible UI.
- [ ] Keep the corridor motion, movement controls, and speed summaries readable and touch-safe on narrow viewports.
...
- [ ] Run Playwright UI verification for `/games/gap-rush` and the Home card.
- [ ] Commit the `Gap Rush` slice in one reviewable unit.
