...
VERIFY_AUTH_REDIRECT_PUBLIC_APP_URL="${VERIFY_AUTH_REDIRECT_PUBLIC_APP_URL:-true}"
...
assert_auth_redirect_uses_public_app_url() {
  ...
  if [[ "${VERIFY_AUTH_REDIRECT_PUBLIC_APP_URL}" != "true" ]]; then
    echo "Skipping auth redirect public app URL verification because VERIFY_AUTH_REDIRECT_PUBLIC_APP_URL=${VERIFY_AUTH_REDIRECT_PUBLIC_APP_URL}."
    return 0
  fi
